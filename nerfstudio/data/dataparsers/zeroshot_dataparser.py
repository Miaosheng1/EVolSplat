...
@dataclass
class Zeroshot(DataParser):
    """Nerfstudio DatasetParser"""

    config: ZeroshotDataParserConfig
    downscale_factor: Optional[int] = None

    def __init__(self, config: ZeroshotDataParserConfig, **kwargs) -> None:
        super().__init__(config, **kwargs)
        self.config.validate()  # Validate configuration when initializing parser
    ...
    def _generate_dataparser_outputs(self, split="train"):
        ...
        elif self.config.waymo:
            num_images = len(image_filenames)
            i_all = np.arange(num_images)
            i_train = np.array([i for i in range(num_images)if (i % 2 == 0)])
            i_eval = np.setdiff1d(i_all, i_train)
            i_eval = i_eval[::2][:-1]
            ply_file_path = data_dir / meta["ply_file_path"]
        else:
            raise ValueError("No dataset type selected. Set either kitti=True or waymo=True")
