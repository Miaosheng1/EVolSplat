...
from nerfstudio.data.datamanagers.utils import read_rgb_filename
...
from nerfstudio.data.datamanagers.utils import get_source_images_from_current_imageid, eval_source_images_from_current_imageid

@dataclass
class SplatDatamanagerConfig(DataManagerConfig):
    _target: Type = field(default_factory=lambda: SplatDatamanager)
    ...
    """The number of source images from the near poses."""


class SplatDatamanager(DataManager, Generic[TDataset]):
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.train_dataset = self.create_train_dataset() # type: ignore
        self.eval_dataset = self.create_eval_dataset() # type: ignore
        ...
        self.num_images_per_scene = len(self.train_dataset) // self.dataparser.config.num_scenes
    ...
    def sample_camId_from_multiscene(self,scene_id,all_pose,train_cameras):
        """
        Return:
        ...
        train_poses = all_pose[start_index:end_index+1,:,:]
        cur_image_filenames = self.train_dataset.image_filenames[start_index:end_index+1]
        cur_depth_filenames = self.train_dataset.metadata['depth_filenames'][start_index:end_index+1]
        
        return image_index, cur_image_filenames, cur_depth_filenames, train_poses, train_cameras[start_index:end_index+1]
    ...
    def dataset_type(self) -> Type[TDataset]:
        ...
        orig_class: Type[SplatDatamanager] = get_orig_class(self, default=None)  # type: ignore
        if type(self) is SplatDatamanager and orig_class is None:
            return default
        if orig_class is not None and get_origin(orig_class) is SplatDatamanager:
            return get_args(orig_class)[0]

        # For inherited classes, we need to find the correct type to instantiate
        for base in getattr(self, "__orig_bases__", []):
            if get_origin(base) is SplatDatamanager:
    ...
    def next_train(self, step: int) -> Tuple[Cameras, Dict]:
        ...
        ## Sample random sceneId for neuralsplat
        scene_id = torch.randint(self.dataparser.config.num_scenes,size=(1,))
        ## Only output rgbs and depths for the current scene_id
        image_idx, cur_rgbs, cur_depths, train_pose, train_cameras = self.sample_camId_from_multiscene(scene_id=scene_id,
                                                                               all_pose=self.train_dataset.cameras.camera_to_worlds,
                                                                               train_cameras = self.train_dataset.cameras,
                                                                               )
    
        tar_image = read_rgb_filename(image_filename=cur_rgbs[image_idx])
        camera = train_cameras[image_idx : image_idx + 1].to(self.device)
        if camera.metadata is None:
            camera.metadata = {}
        camera.metadata["cam_idx"] = image_idx + scene_id * self.num_images_per_scene

        

        ## select the nearest camera pose
        source_images, src_poses, target_pose, source_ids, src_depths = get_source_images_from_current_imageid(image_id = image_idx,
                                                                                                    rgbs=cur_rgbs,
                                                                                                    depths=cur_depths,
                                                                                                    all_pose = train_pose.to(self.device),
        ...
        example = {
                    "source": {
                        ...
                        "intrinsics": intrinsics.repeat(self.config.num_source_image,1,1),
                        "image": source_images.cuda(),
                        "depth": src_depths.cuda(), # type: ignore
                        "source_id":source_ids + scene_id.item() * self.num_images_per_scene
                    },
                    "target": {
                        "extrinsics": target_pose,
                        "intrinsics": intrinsics,
                        "image": tar_image.unsqueeze(0).cuda(),
                        "target_id": image_idx + scene_id.item() * self.num_images_per_scene
                    },
                    "scene_id":scene_id
                }

        return camera, example # type: ignore
    ...
    def next_eval_image(self, step: int) -> Tuple[Cameras, Dict]:
        ...
        For the eval setting, the image_idx is the global index in the eval dataset"""
        image_idx = self.eval_unseen_cameras.pop(random.randint(0, len(self.eval_unseen_cameras) - 1))
        scene_id = image_idx // 2
        _, cur_rgbs, cur_depths, train_pose, _ = self.sample_camId_from_multiscene(scene_id=scene_id,
                                                                            all_pose=self.train_dataset.cameras.camera_to_worlds,
                                                                            train_cameras=self.train_dataset.cameras,
                                                                            )
        camera = self.eval_dataset.cameras[image_idx : image_idx + 1].to(self.device)
        tar_image = read_rgb_filename(image_filename=self.eval_dataset.image_filenames[image_idx])
        ...
        if len(self.eval_unseen_cameras) == 0:
         ...
         ## select the nearest camera pose
        source_images, src_poses, source_ids, src_depths = eval_source_images_from_current_imageid(rgbs=cur_rgbs,
                                                                                        depths=cur_depths,
                                                                                        all_pose = train_pose.to(self.device),
                                                                                        num_select=self.config.num_source_image,
                                                                                        eval_pose=camera.camera_to_worlds
                                                                                        )
        source_ids = np.array(sorted(source_ids))
    ...
    def next_fixed_eval_image(self, image_idx: int) -> Tuple[Cameras, Dict]:
        """ return a assigned cameraID 3D GS outputs, For feedfoward-inference"""
        ...
        camera = self.eval_dataset.cameras[image_idx : image_idx + 1].to(self.device)
         ...
         ## select the nearest camera pose
         ## for eval, the source image collections include all train cameras. tar_id =-1
           ## select the nearest camera pose
        source_images, src_poses, source_ids, src_depths = eval_source_images_from_current_imageid(rgbs=cur_rgbs,
                                                                                        depths=cur_depths,
                                                                                        all_pose = train_pose.to(self.device),
    ...
    def get_interpolate_source_data(self, camera_id, all_cameras: Cameras) -> Tuple[Cameras, Dict]:
        """ return a assigned cameraID 3D GS outputs, For feedfoward-inference"""
        ...
        if len(self.eval_unseen_cameras) == 0:
            self.eval_unseen_cameras = [i for i in range(len(self.eval_dataset))]
        assert len(self.eval_dataset.cameras.shape) == 1, "Assumes single batch dimension"
        camera = all_cameras[camera_id:camera_id+1]
        ...
        source_images, src_poses, source_ids, src_depths = eval_source_images_from_current_imageid(rgbs=cur_rgbs,
                                                                                ...
                                                                                all_pose = train_pose.to(self.device),
                                                                                num_select=self.config.num_source_image,
                                                                                eval_pose=camera.camera_to_worlds.to(self.device)
                                                                                )
        source_ids = np.array(sorted(source_ids) )
        intrinsics = camera.get_normal_intrinsics_matrices().cuda()
        eye = torch.tensor([0., 0., 0., 1.]).unsqueeze(0)
        target_pose = torch.cat([camera.camera_to_worlds,eye.unsqueeze(0)],dim=1)
        example = {
                    "source": {
                        "extrinsics": src_poses,
                    ...
                    "target": {
                        "extrinsics": target_pose.cuda(),
                        "intrinsics": intrinsics,
                        "image": source_images[:1],
                    },
                    "scene_id": scene_id.to(self.device)
                }
