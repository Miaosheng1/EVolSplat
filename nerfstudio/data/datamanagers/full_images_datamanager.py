...
class FullImageDatamanager(DataManager, Generic[TDataset]):
    ...
    def next_train(self, step: int) -> Tuple[Cameras, Dict]:
        ...
        ## add ray_dir
        ray_bundle = self.train_cameras.generate_rays(image_idx)
        direction = ray_bundle.directions
        camera.metadata["ray_dir"] = direction.to(self.device)
        return camera, data
    ...
    def next_eval_image(self, step: int) -> Tuple[Cameras, Dict]:
        ...
        ## add ray_dir
        ray_bundle = self.eval_dataset.cameras.generate_rays(image_idx)
        direction = ray_bundle.directions
        camera.metadata["ray_dir"] = direction.to(self.device)
