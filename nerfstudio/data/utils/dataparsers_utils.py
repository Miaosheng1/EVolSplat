...
def get_train_eval_split_manner(image_filenames: List, train_split_fraction: float):
    """
    Get the train/eval split fraction based on the number of images and the train split fraction.
    ...
    """
    num_images = len(image_filenames)
    i_all = np.arange(0,50,2)
    # i_train = sorted(np.concatenate([np.arange(0,num_images-2,4),np.arange(1,num_images-2,4), np.arange(3,num_images-2,4)],axis=0))
    i_eval = [4,12,20]
    i_train = i_all
