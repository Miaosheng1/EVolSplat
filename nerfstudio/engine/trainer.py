...
class Trainer:
    ...
    def setup_feedforward(self, test_mode: Literal["test", "val", "inference"] = "val") -> None:
        """Setup the Trainer by calling other setup functions.
        ...
        load_step = sorted(int(x[x.find("-") + 1 : x.find(".")]) for x in os.listdir(self.config.load_dir))[-1]
        load_path: Path = self.config.load_dir / f"step-{load_step:09d}.ckpt"  # type: ignore
        
        assert load_path.exists(), f"Checkpoint {load_path} does not exist"
        loaded_state = torch.load(load_path, map_location="cpu")
        self._start_step = loaded_state["step"] + 1
        # load the checkpoints for pipeline
        self.pipeline.load_pipeline(loaded_state["pipeline"], loaded_state["step"])
        CONSOLE.print(f"Done loading Nerfstudio checkpoint from {load_path} and freeze the 3D volume ")
