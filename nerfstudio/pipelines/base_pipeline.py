...
class VanillaPipeline(Pipeline):
    ...
    def get_eval_image_metrics_and_images(self, step: int):
        ...
        outputs = self.model.get_outputs_for_camera(camera,batch)
    ...
    @profiler.time_function
    def get_eval_zeroshot(self, camera_idx: int):
        """This function gets your evaluation loss dict. It needs to get the data
        from the DataManager and feed it to the model's forward function
        ...
        """
        self.eval()
        self.model.update_to_step(step=camera_idx)
        camera, batch = self.datamanager.next_fixed_eval_image(camera_idx) 
        outputs = self.model.get_outputs_for_camera(camera,batch)
        
        return outputs, batch['target']['image']
