...
def apply_XDLab_color_depth_map(depths,
                                scale: float = 5,
                                near_plane =None,
                                far_plane =None,
                                ):
    ...
    #  values = np.clip(depths.flatten() / scale, 0, 1)
     near_plane = near_plane if near_plane is not None else float(np.min(depths))
     far_plane = far_plane if far_plane is not None else float(np.max(depths))

     depths = (depths - near_plane) / (far_plane - near_plane + 1e-10)
     values = np.clip(depths.flatten(), 0, 1)
