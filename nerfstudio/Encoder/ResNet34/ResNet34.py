...
import torchvision
import nerfstudio.Encoder.ResNet34.util as util
from nerfstudio.Encoder.ResNet34.custom_ecnoder import ConvEncoder
...
class SpatialEncoder(nn.Module):
    ...
    def __init__(
        ...
        if self.use_custom_resnet:
            ...
            # )
            self.model = torchvision.models.resnet34(weights='IMAGENET1K_V1',norm_layer=norm_layer)
    ...
    def forward(self, x):
        ...
        if self.use_custom_resnet:
            ...
            ## 通过线性插值，将所有的feature_map （上采样）统一 到 （150,200）的 resoltion 之下
            ## 4 个feature_map 的 channel 分别是 （64,64,128,256）
            ## 然后将4个 feature map concat 到一个volume 之后，是 （512,150,200）
...
if __name__=="__main__":
    encoder = SpatialEncoder(num_layers=3).cuda()
    input_tensor = torch.rand(2,3,176,704).cuda() #[N,C,H,W]
