...
class ModelConfig(InstantiateConfig):
    ...
    config_file: Optional[str] = "config/Neuralsplat.yaml"
    """mannul configuration file"""
...
class Model(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        opts = None
        if os.path.exists(self.config.config_file): #type: ignore
            file = OmegaConf.load('config/Neuralsplat.yaml')
            opts = edict(file)

        self.populate_modules(opts)  # populate the modules
        self.callbacks = None
    ...
    def extract(self,args: str):
        """Extract the confif file parametes form config_file"""
    ...
    def forward(self, ray_bundle: Union[RayBundle, Cameras], images_dict=None) -> Dict[str, Union[torch.Tensor, List]]:
        ...
        if self.collider is not None:
            ...
            ## neural splat method
            images_dict['raybundle'] = self.collider(images_dict['raybundle'])  # type: ignore 

        return self.get_outputs(ray_bundle,images_dict)  # type: ignore
