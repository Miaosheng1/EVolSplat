...
from nerfstudio.fields.background_nerf import Background_NeRF
...
class SplatfactoModelConfig(ModelConfig):
    ...
    """
    background_model: bool = True
    """If True, Use Generalizable background NeRF to build distant view and sky"""
...
class SplatfactoModel(Model):
    ...
    def populate_modules(self):
        ...
        ## Background NeRF configure
        if self.config.background_model:
            self.sampler_bg = LinearDisparitySampler(num_samples=20)
            self.collider = AABBBoxCollider(scene_box=self.scene_box)
            self.bg_field = Background_NeRF(
                        self.scene_box.aabb,
        ...
        if self.step > self.config.refine_every * self.config.reset_alpha_every:
            ...
            if self.step < self.config.stop_screen_size_at:
                ...
                if self.max_2Dsize is not None:
                 ...
                 toobigs = torch.zeros_like(culls)
    ...
    def _get_background_color(self,ray_bundle: RayBundle, source_images, source_extrinsics, intrinsics):
        ray_bundle.nears = ray_bundle.fars 
        ray_bundle.fars = torch.ones_like(ray_bundle.fars) * self.far_bg #type: ignore
        ray_samples_bg = self.sampler_bg(ray_bundle)
        bg_xyz = ray_samples_bg.frustums.get_positions()
        sampled_feat, _ = self.projector.compute(xyz = bg_xyz.reshape(-1,3), 
                                        train_imgs = source_images.squeeze(0),                
                                        train_cameras = source_extrinsics,     
                                        train_intrinsics= intrinsics, 
                                        )   
        sampled_color = sampled_feat.view(-1,9)
        field_outputs_bg = self.bg_field.background_generate(ray_samples=ray_samples_bg,sampled_rgb=sampled_color)
        weights_bg = ray_samples_bg.get_weights(field_outputs_bg[FieldHeadNames.DENSITY])
        rgb = self.renderer_rgb(rgb=field_outputs_bg[FieldHeadNames.RGB], weights=weights_bg)
        del ray_bundle
        return rgb
    ...
    def get_outputs(self, camera: Cameras,batch) -> Dict[str, Union[torch.Tensor, List]]:
        ...
        if self.config.background_model:
            ...
            background = self._get_background_color(ray_bundle=batch['raybundle'],
                                                source_images=source_images.squeeze(0),
                                                source_extrinsics= source_extrinsics,
                                                intrinsics= batch['source']['intrinsics'],
                                                ).reshape(H, W, 3).unsqueeze(0) 
        else:
            background = self._get_background_color(direction=camera.metadata['ray_dir']).reshape(H, W, 3).unsqueeze(0) # type: ignore
        ...
        return {
            ...
            "background": (1 - alpha) * background,  # type: ignore
        }  # type: ignore
    ...
    def get_loss_dict(self, outputs, batch, metrics_dict=None) -> Dict[str, torch.Tensor]:
        ...
        # if batch.get("target") is not None and "mask" in batch['target']:
        #     # batch["mask"] : [H, W, 1]
        #     sky_mask = batch['target']['mask'].to(self.device)
        #     assert sky_mask.shape[:2] == gt_img.shape[:2] == pred_img.shape[:2]
        #     sky_loss = 0.1 * F.binary_cross_entropy_with_logits(outputs['accumulation'],sky_mask.float())
        # elif "mask" in batch:
        #     sky_mask = batch['mask'].to(self.device)
        #     assert sky_mask.shape[:2] == gt_img.shape[:2] == pred_img.shape[:2]
        #     sky_loss = 0.1 * F.binary_cross_entropy_with_logits(outputs['accumulation'],sky_mask.float())
        # else:
        #     sky_loss = torch.tensor(0.0).to(self.device)
