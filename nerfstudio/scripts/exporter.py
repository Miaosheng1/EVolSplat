...
from nerfstudio.models.evolsplat import NeuralSpalatModel
...
class ExportGaussianSplat(Exporter):
    ...
    def write_pcd(
        ...
        with open(filename, "wb") as ply_file:
            ...
            colors = np.concatenate([SH2RGB(map_to_tensors['f_dc_0']),SH2RGB(map_to_tensors['f_dc_1']),SH2RGB(map_to_tensors['f_dc_2'])],axis=1)
    ...
    def main(self) -> None:
        ...
        with torch.no_grad():
            ...
            print(f"Export {count} splats! Waiting.... \n")
            n = count
        ...
        if self.export_pcd:
            filename = self.output_dir / "pcd.ply"
            # ExportGaussianSplat.write_pcd(str(filename), count, map_to_tensors)

            ## visualize the GS scale
            if isinstance(pipeline.model, SplatfactoModel):
                scale_x, scale_y, scale_z = np.exp(map_to_tensors['scale_0']), np.exp(map_to_tensors['scale_1']), np.exp(map_to_tensors['scale_2'])
                opacity = sigmoid(map_to_tensors['opacity'].squeeze()),
            else:
                # scale_x, scale_y, scale_z = map_to_tensors['scale_0'], map_to_tensors['scale_1'], map_to_tensors['scale_2']
                scale_x, scale_y, scale_z = np.exp(map_to_tensors['scale_0']), np.exp(map_to_tensors['scale_1']), np.exp(map_to_tensors['scale_2'])
                opacity = map_to_tensors['opacity'].squeeze()

            scales = np.concatenate([scale_x, scale_y, scale_z],axis=1)
            self.draw_scale_histogram(output_dir=self.output_dir, scales = scales)
            self.draw_opacity_histogram(output_dir=self.output_dir, opa= opacity)
        else:
            ExportGaussianSplat.write_ply(str(filename), count, map_to_tensors)

    def draw_scale_histogram(self, output_dir, scales) -> None:
        
        file_names = ['x','y','z']

        for i, name in enumerate(file_names):
     
            # hist, bins = np.histogram(scales[:,i], bins=10)
            plt.figure(figsize=(6, 6))
    
            # 绘制直方图
            if isinstance(self.pipeline.model, SplatfactoModel):
                plt.hist(scales[:, i], bins=10, edgecolor='k', alpha=0.7,range=(0,0.3))
            else:
                plt.hist(scales[:, i], bins=10, edgecolor='k', alpha=0.7,range=(0,0.3))
                # plt.hist(scales[:, i], bins=10, edgecolor='k', alpha=0.7)

            plt.title(f'Scale: {name}')
    ...
    def draw_opacity_histogram(self, output_dir, opa) -> None:
        plt.figure(figsize=(6, 6))
        name = 'Opacity'
        plt.hist(opa, bins=10, edgecolor='k',color='r', alpha=0.7,range=(0,1))
        plt.title(f' {name}')
        plt.xlabel(f'{name} Values')
        plt.ylabel('Numbers')
